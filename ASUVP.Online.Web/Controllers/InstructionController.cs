...
namespace ASUVP.Online.Web.Controllers
{
    ...
    public class InstructionController : Controller
    {
        ...
        public InstructionController(IInstructionService service, IUserNotificationService notificationService)
        {
            _service = service;
        ...
        public JsonResult DeleteInstructions(Guid[] keys)
        {
            ...
            foreach (var item in keys)
            {
                ...
                var idNotificstion = _notificationService.AddUserNotification(AuthManager.User.UserId, AuthManager.User.CompanyId, AuthManager.User.UserId, AuthManager.User.CompanyId, "Инструкция " + _service.GetInstruction(item).TemplateName + " был удалена", "Instruction|details|", (Guid)_service.GetInstruction(item).Id);
